...
### Статусы материалов в ROADMAP

В [ROADMAP.md](./ROADMAP.md) автоматически отмечено, для каких подтем уже есть файлы в `lessons/`:

| Маркер | Значение |
|--------|----------|
| 📚 | Полный урок: `1.lection-eng.md` … `5.test-yourself-answers.md` |
| 📝 | Частичный: папка есть, но не все обязательные шаги |
| — | Папка `lessons/{id}/` отсутствует или пуста |

Где смотреть:
- **Шапка ROADMAP** — сводка: сколько 📚 / 📝 / — по всему curriculum
- **Колонка «Урок»** в таблице подтем узла
- **`> **Материалы:** 📚 N/M`** под заголовком узла

**Обновление:** `npm run parse-roadmap` (корень репо) сканирует `lessons/`, пересчитывает маркеры в `ROADMAP.md` и `app/src/data/roadmap.json`. Маркеры в таблицах **не править вручную** — только через скрипт.

После **создания или существенного дополнения** файлов урока AI **обязан** выполнить `npm run parse-roadmap`, чтобы ROADMAP и desktop app показывали актуальный статус.
...
6. Запустить **`npm run parse-roadmap`** из корня репо — обновить маркеры 📚/📝/— в `ROADMAP.md` и `roadmap.json` для app (см. «Статусы материалов в ROADMAP»).
7. Сообщить ученику порядок прохождения и ожидаемое время.
...
|---|---|
| «Заполни урок 0.1.1» | Создать все файлы урока в `lessons/0.1.1/`; лекции — **narrative prose** (см. спецификацию `1`/`2`); затем **`npm run parse-roadmap`** |
| «Create lesson for 0.1.1» | То же |
| «Заполни уроки 0.1.1–0.1.5» | То же для каждой подтемы; **один** `npm run parse-roadmap` в конце |
...
- ❌ Не править маркеры 📚/📝/— в `ROADMAP.md` вручную — только `npm run parse-roadmap`
- ❌ Не забывать `npm run parse-roadmap` после генерации или дополнения файлов в `lessons/`
...
1. Sidebar: фазы → узлы → подтемы (из `roadmap.json`; генерируется **`npm run parse-roadmap`** из `ROADMAP.md` + `lessons/`)
...
- Если в app badge **empty** — урок ещё не создан; AI создаёт файлы в `lessons/{id}/` и запускает **`npm run parse-roadmap`**
- После создания урока app подхватывает файлы автоматически (перезагрузка шага); sidebar использует `hasLesson` из `roadmap.json`
