...
7. **Без воды** — плотный, структурированный контент; примеры кода — минимальные, но показательные.
...
```
1. Warm-up (5 мин)        — 1 вопрос по прошлой подтеме (из 3.test-yourself)
2. Lecture (30–45 мин)    — ученик читает 1.lection.md
3. Summary (10 мин)       — ученик читает 2.summary.md, задаёт вопросы AI
4. Self-test (15 мин)     — 3.test-yourself.md БЕЗ подглядывания в answers
5. Review (10 мин)        — сверка с 4.test-yourself-answers.md + разбор ошибок
6. Exercises (15–30 мин)  — 5.exercises.md (если есть); после pass по тесту
7. Gate                    — ≥80% → подтема ✅; иначе повтор weak spots
```
...
| Артефакт | Язык | Зачем |
|---|---|---|
| `1.lection.md` | **English** | Полная лекция; привыкание к industry English |
| `2.summary.md` | **Русский** (термины — English) | Быстрое повторение перед тестом |
| `3.test-yourself.md` | **Русский** | Вопросы ученику |
| `4.test-yourself-answers.md` | **Русский** | Разбор ответов |
| `5.exercises.md` | **English** (описание задач) | Практика после теста (опционально) |
| Общение AI ↔ ученик | **Русский** | Объяснения, review, feedback |
| Код, API, термины | **English** | Как в индустрии |

- Технические термины при первом упоминании в `2.summary.md` — с кратким пояснением.
...
lessons/
  0.1.1/                          ← подтема из ROADMAP (узел.подтема)
    1.lection.md                  ← 1. полная лекция (English)
    2.summary.md                  ← 2. выжимка (русский)
    3.test-yourself.md            ← 3. тест (без ответов)
    4.test-yourself-answers.md    ← 4. ответы + разбор
    5.exercises.md                ← 5. практические задания (опционально)
  0.1.2/
    ...
...
3. Читает `1.lection.md` (основной материал).
4. Читает `2.summary.md` (закрепление).
5. Проходит `3.test-yourself.md` **не открывая** `4.test-yourself-answers.md`.
6. Сверяет ответы, разбирает ошибки с AI.
7. Выполняет `5.exercises.md` (если файл создан и тест пройден).
8. При ≥80% — отмечает подтему ✅ в `PROGRESS.md`, переходит к следующей.
...
3. Создать **все обязательные файлы** (см. ниже).
4. Создать `5.exercises.md`, если тема допускает практику (Git, SQL, код) — см. критерии.
5. **Не** открывать ученику `4.test-yourself-answers.md` до прохождения теста (файл создаётся сразу в репо — ученик simply не открывает его).
6. Сообщить ученику порядок прохождения и ожидаемое время.

### Критерий «подтема пройдена» (gate)

- [ ] Прочитаны `1.lection.md` и `2.summary.md`
- [ ] Self-test: **≥80%** без подглядывания (`3` → `4`)
- [ ] Выполнены упражнения из `5.exercises.md` (если файл есть) — или явно пропущены
...
### 1. `1.lection.md` (обязательный) — Full Lecture

**Язык: English.** Это основной учебный материал — максимально подробный.

**Объём:** зависит от глубины в ROADMAP:
| Глубина | Ориентир |
|---|---|
| понимание | 800–1500 слов |
| практика | 1000–2000 слов + больше examples |
| **глубоко** | 1500–3000 слов + internals, diagrams |
...
**Требования к качеству `1.lection.md`:**

- Каждый abstract concept → concrete example или analogy.
- Code/commands — **working**, copy-pasteable where possible.
- Минимум 1 diagram (mermaid или ASCII) для non-trivial flow.
- Не ссылаться на «you already know X» без recap в 1–2 предложения.
- Depth `**глубоко**` → секция **Under the Hood** обязательна и не может быть поверхностной.
- Не дублировать summary — lection = complete story; summary = compressed.

---

### 2. `2.summary.md` (обязательный) — Выжимка
...
Summary **не заменяет** lection — это шпаргалка для повторения перед тестом.

---

### 3. `3.test-yourself.md` (обязательный) — Self-Test
...
> ⛔ Не открывай `4.test-yourself-answers.md` до завершения.
...
- ❌ **Никаких ответов** в этом файле — только в `4.test-yourself-answers.md`.
...
### 4. `4.test-yourself-answers.md` (обязательный) — Answers & Explanations
...
- <80%:     вернись к `1.lection.md` + `2.summary.md`
...
### 5. `5.exercises.md` (опциональный, но рекомендуется)

Создавай, когда подтема **практическая** или имеет hands-on component:

| Создавать `5.exercises.md` | Примеры подтем |
|---|---|
...
> Complete after reading `1.lection.md` and passing `3.test-yourself.md`. Ask AI for hints, not full solutions.
...
**Solutions:** отдельный файл `5.exercises-solutions.md` — создавай **только по запросу** ученика после попытки.
...
|---|---|
| «Заполни урок 0.1.1» | Создать все файлы урока в `lessons/0.1.1/` |
...
| «Дай solutions к exercises 0.1.1» | Создать/показать `5.exercises-solutions.md` |
...
- ❌ Не класть ответы в `3.test-yourself.md` — только в `4.test-yourself-answers.md`
- ❌ Не давать `5.exercises-solutions.md` до попытки ученика
- ❌ Не создавать урок, пока ученик не создал папку `lessons/{id}/`
- ❌ Не объединять несколько подтем в один `1.lection.md`
- ❌ Не писать `1.lection.md` на русском — только English
...
2. Шаги подтемы: **lection → summary → test → answers → exercises**
...
Вкладка **Review** — flashcards из `3.test-yourself.md` + `4.test-yourself-answers.md` **только пройденных** подтем (status = completed).
...
- **Roadmap:** см. [ROADMAP.md](./ROADMAP.md) — 20 фаз, ~153 узла, ~1760 подтем (Middle: 0–9 · Middle+: 10–19)
